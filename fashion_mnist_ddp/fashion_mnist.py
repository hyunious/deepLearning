...
# TODO : epoch 별 validation accuracy 를 csv 로 저장하는 함수를 구현한다.
# epoch 시작 시점에 호출하여 csv 파일을 오픈 한다.
...
# epoch 종료 시점에 호출하여 epoch processing time 과 accuracy 를 csv 파일에 저장한다.
...
def worker(local_rank, args):
    ...
    data_filepath = "training_data/{}ranks-{}bs-{}lr.csv".format(
        WORLD_SIZE, args.batch_size, args.base_lr)
    ...
    for epoch in range(args.epochs):
        ...
        if global_rank == 0:
            ...
            ## Note that you will have to convert the validation accuracy to a standard python number
            ## using torch.Tensor.item.
            on_epoch_end(data_filepath, epoch_time, val_acc[-1])
